...
## 2025-10-21 libx-docsリポジトリ廃止の決定（Phase 3-5）

### 決定内容

**libx-docsリポジトリを廃止する**ことを決定し、Phase 3完了時（2025-11-15目標）までに移行作業を完了する。

### 背景

Phase 3-5（移行作業計画）において、libx-docsリポジトリの今後の役割を検討した結果、以下の理由により廃止が適切と判断した。

#### 1. 新レジストリ形式への移行完了

- 新レジストリ形式（`registry/docs.json`）で全てのコンテンツを管理可能
- libx-dev/apps/libx-docs に全コンテンツが統合される
- 外部リポジトリを維持する必要性がなくなる

#### 2. 二重管理のコスト削減

- libx-docsリポジトリとlibx-dev/apps/libx-docsの同期コスト
- syncHash管理、同期スクリプトのメンテナンスコスト
- 同期状態の不一致リスク（R6: リスクログ参照）
...
3. **最終バックアップの作成**
   - libx-docsリポジトリの完全バックアップ（Git clone --mirror）
   - Artifactsへの保存（60日保持）
   - ローカルバックアップの作成
...
1. **新レジストリ形式への変換**
   - Migration Workflow実行（dry-run: true）
   - 変換結果のレビュー
   - 本番変換実行（dry-run: false）
...
1. **README更新**
   - libx-docsリポジトリのREADMEに廃止通知を追加
   - 移行先（libx-dev/apps/libx-docs）への案内
   - アーカイブ日時の記載

2. **リポジトリのアーカイブ化**
   - GitHubでリポジトリをアーカイブ（Read-only化）
   - トピックに`archived`タグ追加
   - Descriptionに廃止日時を追加

3. **ドキュメント更新**
   - libx-devのREADME更新（libx-docs言及箇所の削除）
   - CLAUDE.md更新（libx-docs同期機能の削除）
   - 関連スクリプトの削除または非推奨化
...
```
2025-10-21: 廃止決定（Phase 3-5計画策定）
2025-11-11: 最終同期確認開始
2025-11-12: 移行作業開始（Migration Workflow実行）
2025-11-14: 移行作業完了
2025-11-15: リポジトリアーカイブ化、Phase 3完了
```
...
- ⚠️ 既存のlibx-docsリポジトリへの参照がリンク切れ
  - 緩和策: アーカイブ化（Read-only化）により履歴は保持
- ⚠️ 外部からのコントリビューションの受付先が変更
  - 緩和策: README更新で新しい受付先を明示
...
**緩和策**:
- 最終同期の徹底（syncHash確認、手動レビュー）
- 複数バックアップの作成（Git mirror、Artifacts、ローカル）
- Migration WorkflowでのDry-run確認
- ビルド・デプロイテストの実施

詳細は [移行リスクログ](./status/migration-risks.md) を参照。

### 代替案との比較

#### 代替案A: libx-docsリポジトリを維持
...
**結論**: ❌ 不採用（コストが高く、新システムとの整合性確保が困難）

#### 代替案B: libx-docsリポジトリを縮小
...
**結論**: ❌ 不採用（中途半端な状態を避けるため）

#### 採用案: libx-docsリポジトリを完全廃止
...
**結論**: ✅ 採用（メリットがデメリットを大きく上回る）

### 決定の根拠

1. **フェーズ0の合意事項（決定3）との整合性**
   - 「移行期間中はソース置き場として維持し、フェーズ3完了時に継続是非を見直す」
   - Phase 3-5でレビューを実施し、廃止を決定

2. **プロジェクト原則との整合性**
   - 原則2: 共有資産の最大活用 → モノレポで一元管理
   - 原則4: コンテンツ運用原則 → レジストリで宣言的に管理
   - 原則6: コラボレーションとナレッジ共有 → 単一リポジトリで情報集約

3. **技術的実現可能性**
   - 新レジストリ形式で全コンテンツを管理可能
   - Migration Workflowによる自動移行が可能
   - バックアップ・ロールバック機能の整備済み

4. **リスク管理**
   - Phase 3-4で構築したCI/自動化インフラにより安全な移行が可能
   - 複数バックアップによりデータ損失リスクを最小化

### 承認・レビュー

- **決定者**: テックリード（TBD）
- **決定日**: 2025-10-21
- **レビュー参加者**: 開発チーム、コンテンツリード
- **次回レビュー**: Phase 3完了時（2025-11-15）
...
- [Phase 3.0調査レポート](./status/phase-3-0-preparation-report.md)

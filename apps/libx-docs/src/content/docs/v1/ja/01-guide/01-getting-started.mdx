---
title: "Libxをはじめよう"
description: "MDXの基本、構成、最初の1ページを作るまで"
---
...
このガイドはLibxでドキュメントを管理するための方法を学べます。
...
- **libx-core**: libx-dev からコア機能を抽出したリポジトリ
- **libx-docs**: libx-dev にドキュメントを提供するためのリポジトリ

## セットアップ

Libxは[libx.dev](https://libx.dev)のために設計されていますが、libx-coreを使うことで独自のドキュメントサイトを作成できます。
...
Libxのドキュメントは共有UIコンポーネントを使うができます。
...
<Alert type="info">
  <Icon name="info" /> 共有コンポーネントはMDXでもそのまま使えます。
</Alert>
```

## 次のステップ

次は「[セットアップと構成](/v1/ja/01-guide/02-setup-and-structure)」へ進み、プロジェクトの全体像とベースパスの扱いを確認しましょう。
